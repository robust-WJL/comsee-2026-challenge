...
The request is `{"frame":"<hex>"}`. Transmitted payloads are limited to 4,096
bytes. Domain statuses are `HEADER`, `MAGIC`, `VERSION`, `FLAGS`, `PAYLOAD_LIMIT`,
`FRAME_LENGTH`, `CHECKSUM`, `TLV_LENGTH`, `FIELD_LENGTH`, and `TEXT_ASCII`; errors
have a null result and expose no partial fields.

Success is `{"status":"OK","result":{"fields":[...]}}`. Preserve order and
duplicates. Type 1 is `{"type":1,"kind":"COUNTER","value":u32}`; type 2 is
`{"type":2,"kind":"MEASUREMENT","value":i32}` using big-endian two's
complement; type 3 is `{"type":3,"kind":"TEXT","value":"..."}`. Unknown
types are `{"type":N,"kind":"BYTES","value":"<lowerhex>"}`.
