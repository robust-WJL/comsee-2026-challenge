...
The request is `{"input":"<hex>"}`, containing at most 65,536 raw CSV bytes.
Input is ASCII; NUL is ordinary field data. Limits are 4,096 decoded bytes per
field, 32 fields per row, and 128 rows. Domain statuses are `INPUT_TOO_LONG`,
`NON_ASCII`, `BARE_CR`, `UNEXPECTED_QUOTE`, `TRAILING_TEXT`, `FIELD_TOO_LONG`,
`FIELD_LIMIT`, `ROW_LIMIT`, and `UNTERMINATED_QUOTE`. Errors have a null result.
Success is
`{"status":"OK","result":{"rows":[["field",...],...]}}`.
