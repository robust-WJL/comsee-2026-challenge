...
The request is `{"input":"<hex>"}` with at most 4,096 decoded bytes. Longer input
returns `INPUT_TOO_LONG` with a null result. Success is
`{"status":"OK","result":{"encoded":"<lowercase hex>"}}`.
