...
The request is `{"input":"<hex>"}` with at most 4,096 decoded bytes. At most 1,024
signed 32-bit values may be produced. Domain statuses are `INPUT_TOO_LONG`,
`LIMIT`, `TRUNCATED`, `OVERFLOW`, and `NONCANONICAL`; errors have a null result and
do not return a decoded prefix. Success is
`{"status":"OK","result":{"values":[signed integers...]}}`.
