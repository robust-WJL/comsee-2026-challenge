...
The request is `{"input":"<hex>"}` and must decode to exactly four bytes. Other
lengths return `INPUT_LENGTH` with a null result. Success is
`{"status":"OK","result":{"version":integer,"kind":integer,`
`"flags":integer,"measurement":integer}}`.
