...
# C13 — Integer-expression evaluator
...
The request is
`{"expression":"<string>","variables":[{"name":"<string>","value":i64},...]}`.
Expressions are limited to 4,096 decoded bytes, 32 variables, and nesting depth 64.
Variable names match `[A-Za-z_][A-Za-z0-9_]*`. Domain statuses are `LIMIT`,
`VARIABLE_NAME`, `DUPLICATE_VARIABLE`, `SYNTAX`, `DEPTH`, `LITERAL_RANGE`,
`UNKNOWN_VARIABLE`, `DIVIDE_BY_ZERO`, and `OVERFLOW`; errors have a null result.
Success is `{"status":"OK","result":{"value":i64}}`.
