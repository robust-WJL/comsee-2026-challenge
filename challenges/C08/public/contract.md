...
The request is
`{"sets":integer,"ways":integer,"block_size":integer,"addresses":[integers...]}`.
Set counts are one of `1,2,4,8,16`, ways are `1..8`, block sizes are one of
`4,8,16,32,64`, and addresses are unsigned 32-bit integers. At most 4,096
addresses are allowed. Domain statuses are `INVALID_CONFIG`, `LIMIT`, and
`INVALID_ADDRESS`; an invalid address result includes `{"index":i}`. Success is
`{"status":"OK","result":{"accesses":[...],
"hits":h,"misses":m,"evictions":e}}`. A hit event is exactly
`{"status":"HIT"}`. A miss event is exactly
`{"status":"MISS","evicted":null}` for a cold fill or contains the evicted
unsigned block-base address.
