...
The request is `{"n":integer,"edges":[[u,v],...]}`. Tasks are numbered
`0..n-1`; `[u,v]` means `u` must finish before `v`. The limits are 64 tasks and
512 supplied edges. Domain statuses are `INVALID_N`, `LIMIT`, `INVALID_EDGE`, and
`CYCLE`; an invalid edge result includes `{"index":i}`. Success is
`{"status":"OK","result":{"order":[task IDs...]}}`.
